...
- $\|\bx\|_1 = \sum_{i=1}^n |x_i|$ \quad (*Manhattan norm* — sum of absolute values)
  
- $\|\bx\|_2 = \sqrt{\sum_{i=1}^n x_i^2} = \sqrt{\bx^T\bx}$ \quad (*Euclidean norm*)
  
- $\|\bx\|_\infty = \max_{1\leq i\leq n}|x_i|$ \quad (*max norm* — limit as $p \to \infty$)
...
::: {.callout-note icon=false}
## Theorem: Equivalence of Vector Norms
...
1. Prove the triangle inequality for $\|\cdot\|_2$ using Cauchy-Schwarz: expand $\|\bx+\by\|_2^2 = (\bx+\by)^T(\bx+\by)$.
  
1. In NumPy, `np.linalg.norm(x, ord=p)` computes $\ell^p$ norms. For a random $\bx \in \fR^{10}$, verify numerically that $\|\bx\|_\infty \leq \|\bx\|_2 \leq \sqrt{10}\|\bx\|_\infty$.
  
1. Show that $\|\bx\|_\infty = \lim_{p\to\infty}\|\bx\|_p$. (*Hint: factor out the maximum entry.*)
...
A function $\|\cdot\| : \fR^{m\times n} \to \fR$ is a *matrix norm* if for all $\bA, \bB \in \fR^{m\times n}$ and all $c \in \fR$:
...
A square-matrix norm is additionally *submultiplicative* if $\|\bA\bB\| \leq \|\bA\|\|\bB\|$.
:::


::: {.callout-note icon=false}
## Definition: Induced (Operator) Norm
Given vector norms $\|\cdot\|_u$ on $\fR^m$ and $\|\cdot\|_v$ on $\fR^n$, the *induced* (or *operator*) norm of $\bA \in \fR^{m\times n}$ is
...
::: {.callout-note icon=false}
## Definition: $L_1$ Norm (Maximum Absolute Column Sum)
...
::: {.callout-note icon=false}
## Definition: $L_\infty$-Norm (Maximum Absolute Row Sum)
...
::: {.callout-note icon=false}
## Theorem: Spectral Norm via Singular Values
$\|\bA\|_2 = \sigma_{\max}(\bA)$.
:::
...
The *Frobenius norm* treats $\bA$ as a vector in $\fR^{mn}$:
$$\|\bA\|_F = \sqrt{\sum_{i=1}^m \sum_{j=1}^n a_{ij}^2}.$$
:::


::: {.callout-note icon=false}
## Theorem: Frobenius Norm via Trace and Singular Values
...
::: {.callout-note icon=false}
## Proposition: Consistency with Vector Norms
...
1. For $n\times n$ matrices, prove $\|\bA\|_2 \leq \sqrt{n}\|\bA\|_\infty$ and $\|\bA\|_\infty \leq \sqrt{n}\|\bA\|_2$.
  
1. Using NumPy, verify these inequalities for 1000 random $5\times 5$ matrices drawn from `np.random.randn`. Plot histograms of the ratios $\|\bA\|_F/\|\bA\|_2$ and $\|\bA\|_\infty/\|\bA\|_2$.
