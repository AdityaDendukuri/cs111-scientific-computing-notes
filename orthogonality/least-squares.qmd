...
## Definition: Overdetermined Linear System
...
Given $\bA \in \fR^{m\times n}$ and $\bb \in \fR^m$, the *least squares problem* is
...
The vector $\hat{\bx}$ minimizes $\|\bA\bx - \bb\|_2^2$ if and only if it satisfies the *normal equations*
...
::: {.callout-note icon=false}
## Theorem: Condition Number Squaring
...
::: {.callout-note icon=false}
## Definition: Regularized Least Squares (Ridge Regression)
...
$\{1, t, t^2, ..., t^n\}$, the *Vandermonde matrix* is
$$\bV =
  \begin{pmatrix}
  ...
  \end{pmatrix}
  \in \fR^{m \times (n+1)}.$$
The polynomial fitting problem with monomial coefficients $\bc = (c_0, ..., c_n)^T$
...
1. Write down the Vandermonde design matrix $\bA \in \fR^{6\times 3}$ explicitly.
  
1. Solve the normal equations by hand (or with NumPy). State the fitted polynomial.
  
1. Compute the residual $\|\bA\hat{\bx} - \bb\|_2$ and the coefficient of determination $R^2 = 1 - \|\bA\hat{\bx} - \bb\|_2^2/\|\bb - \bar{b}\mathbf{1}\|_2^2$.
  
1. Now solve via QR using `np.linalg.lstsq` and verify you get the same coefficients. Compare $\kappa_2(\bA)$ with $\kappa_2(\bA^T\bA)$ to illustrate condition number squaring.
...
1. Compute the least squares fit without regularization. Plot the fitted polynomial. Does it overfit?
  
1. Compute $\hat{\bx}_\lambda$ for $\lambda = 10^{-4}, 10^{-2}, 1$. How does the fit change?
  
1. Plot $\|\hat{\bx}_\lambda\|_2$ and the residual $\|\bA\hat{\bx}_\lambda - \bb\|_2$ as functions of $\lambda$ (the bias-variance tradeoff curve). Which $\lambda$ gives the best fit?
...
1. Solve via normal equations: `np.linalg.solve(A.T @ A, A.T @ b)`.
  
1. Solve via QR: `np.linalg.lstsq(A, b)`.
  
1. Compare the errors $\|\hat{\bx} - \bx_{\mathrm{true}}\|_2$. Which method is more accurate? Explain in terms of condition numbers.
