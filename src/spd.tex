...
\textbf{(Applications)}
\begin{enumerate}
    \item \textbf{Covariance:} Generate \texttt{X = np.random.randn(100, 4)}. Verify \texttt{X.T @ X} is SPD via Cholesky.
    \item \textbf{Gram Matrix:} Prove that $\mathbf{G} = \bX^T\bX$ is at least positive semi-definite for any $\bX$, and PD if $\bX$ has full column rank.
\end{enumerate}
}
